...
export function useSidebarCollapsed() {
  ...
  const setSidebarCollapsedStore = useSettingsStore((state) => state.setSidebarCollapsed)
...
export function usePersistedTimeRange() {
  ...
  const setTimeRangeStore = useSettingsStore((state) => state.setPerformanceDashboardTimeRange)
  const { debouncedUpdate } = useUpdateUISettings()
