  ...
  useEffect(() => {
    if (query.data) {
      loadSettings({
        ...
        performanceDashboardTimeRange: query.data.performanceDashboardTimeRange as TimeRange,
      })
    }
 ...
 * - Silent failures (settings still work via offline)
 */
export function useUpdateUISettings() {
  const queryClient = useQueryClient()
  const setIsSyncing = useSettingsStore((state) => state.setIsSyncing)
